...
- **Tests passed locally but timed out in CI against Open-Meteo.** On the first CI run, the five Open-Meteo tests that wait for a real forecast failed with read timeouts, while the same tests passed reliably from a home connection. Public APIs can respond more slowly to CI runners (data-center IPs) than to a normal client. The fix had two parts. First, the shared client retries transient network errors (timeouts and connection failures) with a longer timeout, while still NOT retrying real HTTP error responses, since those are answers the tests need to see. Second, when Open-Meteo is unreachable entirely, the weather tests skip rather than fail (see `tests/weather/conftest.py`): a skip honestly reports "couldn't verify this right now," which is different from "this is broken." The PokeAPI tests are reliable from CI and run normally. A natural next step would be to mock the external responses in CI for full determinism while keeping the live tests for true integration coverage.
